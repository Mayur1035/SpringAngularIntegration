  ...
  //  @Test
    ...
    public void updateNoteTest() {
        ...
        while (iterator.hasNext()) {
            note = (Note) iterator.next();
            if (note.getNoteId() == 1)
                note.setText("Mumbai Indians vs RCB match scheduled  for 4 PM is cancelled");
        }
        noteUser.setNotes(allNotes);
        noteRepository.save(noteUser);
        allNotes = noteRepository.findById("Jhon123").get().getNotes();
        Assert.assertEquals("Mumbai Indians vs RCB match scheduled  for 4 PM is cancelled", allNotes.get(0).getText());
    }
