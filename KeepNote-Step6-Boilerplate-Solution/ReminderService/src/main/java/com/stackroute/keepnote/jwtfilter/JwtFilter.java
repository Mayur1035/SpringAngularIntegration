...
import static com.stackroute.keepnote.utils.CommonConstants.*;
...
public class JwtFilter extends GenericFilterBean {
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
		...
		final String authHeader = req.getHeader(AUTH_STR);

		if (OPTION_STR.equals(req.getMethod())) {
			...
			final Claims claims = Jwts.parser().setSigningKey(SECRET_KEY_STR).parseClaimsJws(token).getBody();
			req.setAttribute(CLAIMS_STR, claims);
			chain.doFilter(request, response);
		}
